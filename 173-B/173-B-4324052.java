...
public static void main(String[] args) throws IOException
{
  ...
  while(!q.isEmpty())
  {
      ...
      if(at<n)
      {
          for(int i = 0; i<m; i++)
          {
              if(data[at][i] && visited[n+i] > 1+visited[at])
              {
                  visited[n+i] = visited[at]+1;
                  q.add(n+i);
              }
          }
      }
      else
      {
          for(int i = 0; i<n; i++)
          {
              if(data[i][at-n] && visited[i] > 1+visited[at])
              {
                  visited[i] = visited[at]+1;
                  q.add(i);
              }
          }
      }
  }
  out.println(visited[0] == 987654321 ? -1 : visited[0]);
  out.close();
}
