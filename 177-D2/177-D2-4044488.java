...
public static void main(String[] args) throws IOException
{
  ...
  mod = c;
  int[] data = new int[n], qs = new int[m];
  for(int i = 0; i<n; i++) data[i] = input.nextInt();
  for(int i = 0; i<m; i++) qs[i] = input.nextInt();
  int[] diff = new int[n];
  for(int i = 1; i<n; i++) diff[i] = (int) ((mod+data[i]-data[i-1])%mod);
  diff[0] = data[0];
  for(int i = 0; i<m; i++)
  {
      diff[i] = (int) ((diff[i]+qs[i])%mod);
      if(i+n-m+1 < n) diff[i+n-m+1] = (int) ((diff[i+n-m+1]+mod-qs[i])%mod);
  }
  int at = diff[0];
  out.print(diff[0]);
  for(int i =1; i<n; i++) out.print(" "+(at=(int) ((at+diff[i])%mod)));
  out.close();
