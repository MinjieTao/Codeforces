...
public static void main(String[] args) throws IOException
{
  ...
  for(int i = 0; i<3; i++) dists[i] = data[i].dist(data[(i+1)%3]);
  Arrays.sort(dists);
  if(Math.abs(dists[0]*dists[0] + dists[1]*dists[1] - dists[2]*dists[2]) < 1e-6)
      out.println("RIGHT");
  else
  {
      boolean found = false;
      ...
      for(int p = 0; p<3; p++)
      {
          for(int k = 0; k<4; k++)
          {
              Point[] np = new Point[3];
              for(int i = 0; i<3; i++) np[i] = new Point(data[i].x,data[i].y);
              np[p].x+=dx[k];
              np[p].y+=dy[k];
              dists = new double[3];
              for(int i = 0; i<3; i++) dists[i] = np[i].dist(np[(i+1)%3]);
              Arrays.sort(dists);
              if(dists[0]>0 && Math.abs(dists[0]*dists[0] + dists[1]*dists[1] - dists[2]*dists[2]) < 1e-6)
              {
                  //out.println(p+" "+k+" "+dists[0]+" "+dists[1]+" "+dists[2]);
                  found = true;
              }
          }
      }
      out.println(found?"ALMOST":"NEITHER");
  }
  out.close();
