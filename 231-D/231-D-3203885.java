...
public static void main(String[] args) throws IOException
{
    ...
    int atx = input.nextInt(), aty = input.nextInt(), atz = input.nextInt(), x = input.nextInt(), y = input.nextInt(), z = input.nextInt();
    int[] data = new int[6];
    for(int i = 0; i<6; i++) data[i] = input.nextInt();
    int res = 0;
    if(atx>x) res+=data[5];
    else if(atx<0) res+=data[4];
    if(aty>y) res+=data[1];
    else if(aty<0) res+= data[0];
    if(atz>z) res+=data[3];
    else if(atz<0) res+=data[2];
    out.println(res);
    out.close();
}
