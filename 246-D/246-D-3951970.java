...
public static void main(String[] args) throws IOException
{
    ...
    HashSet<Integer>[] adj = new HashSet[100001];
    for(int i = 0; i<100001; i++) adj[i] = new HashSet<Integer>();
    for(int i = 0; i<m; i++)
    {
        int a = colors[input.nextInt()-1], b = colors[input.nextInt()-1];
        if(a==b) continue;
        adj[a].add(b);
        adj[b].add(a);
    }
    ...
    for(int i = 0; i<100001; i++){
        if(has[i] && adj[i].size()>res) best = i;
        if(has[i]) res = Math.max(res, adj[i].size());
    }
    out.println(best);
    out.close();
}
